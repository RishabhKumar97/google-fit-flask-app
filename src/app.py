...
class Config:
    ...
    def gather_files() -> List[str]:
        ...
        """
        return [
            os.path.join(Config._metric_files_dir, file_name)
            for file_name in os.listdir(Config._metric_files_dir)
        ]
...
class MetricDataFrame:
    ...
    def _get_metric_data(mapping: Dict[str, str], metric: str) -> pd.DataFrame:
        ...
        """
        try:
            metric_df = pd.read_parquet(mapping[metric])
    ...
    def get_all_metrics_df(mapping: dict[str, str]) -> pd.DataFrame:
        ...
        convert_to_date_col = (
            lambda col: "date" if col in ["date", "month", "week"] else col
        )
        ...
        with ThreadPoolExecutor(max_workers=len(mapping)) as e:
            ...
            for k in mapping.keys():
                futures.append(
                    e.submit(MetricDataFrame._get_metric_data, mapping, metric=k)
                )
        for future in as_completed(futures):
            ...
            metric_df["value"] = metric_df[
                [
                    ...
                    if col not in ["entity", "date", "metric"]
                ]
            ]
...
def get_json_from_data(
    ...
    date_error_msg = {
        "status": "error",
        "message": "date must be provided as a string in ISO format (YYYY-MM-DD).",
    }
    entity_error_msg = {
        "status": "error",
        "message": "one or more entities provided do not exist.",
    }
    ...
    df_copy = df_copy[
        (entity_filter & start_date_filter & end_date_filter & metric_filter)
    ]
